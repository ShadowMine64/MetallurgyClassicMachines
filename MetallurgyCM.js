/* BetterStorage API developed by @DAW330073 */
var activity=com.mojang.minecraftpe.MainActivity.currentMainActivity.get();var chestName;var display=new android.util.DisplayMetrics();com.mojang.minecraftpe.MainActivity.currentMainActivity.get().getWindowManager().getDefaultDisplay().getMetrics(display);var Inventory={};var BetterStorage={};var GUI={};var storageBlocks={};var invID=null;Inventory.defineBlock=function(id,name,texture,materialSourceId,opaque,rendertype,size){DefineInventory(id,name,texture,materialSourceId,opaque,rendertype,size);}Inventory.setItem=function(id,texture,type,name,stacksize,size){SetInventory(id,texture,type,name,stacksize,size);}ModPE.saveWorldFile = function(filename, content) {try {java.io.File(android.os.Environment.getExternalStorageDirectory().getPath() + "/games/com.mojang/minecraftWorlds/" + Level.getWorldDir() + "/").mkdirs();var newFile = new java.io.File(android.os.Environment.getExternalStorageDirectory().getPath() + "/games/com.mojang/minecraftWorlds/" + Level.getWorldDir() + "/", filename);newFile.createNewFile();var outWrite = new java.io.OutputStreamWriter(new java.io.FileOutputStream(newFile));outWrite.append(content);outWrite.close();} catch (err) {print(err);}};ModPE.loadWorldFile = function(filename) {var content = "";if (java.io.File( android.os.Environment.getExternalStorageDirectory().getPath() + "/games/com.mojang/minecraftWorlds/" + Level.getWorldDir() + "/" + filename).exists()) {var file = new java.io.File(android.os.Environment.getExternalStorageDirectory().getPath() + "/games/com.mojang/minecraftWorlds/" + Level.getWorldDir() + "/" + filename)fos = new java.io.FileInputStream(file),str = new java.lang.StringBuilder(),ch;while ((ch = fos.read()) != -1) {str.append(java.lang.Character(ch));}content = String(str.toString());fos.close();}return content;};ModPE.readFile = function(filename) {var content = "";if (java.io.File(filename).exists()) {var file = new java.io.File(filename)fos = new java.io.FileInputStream(file),str = new java.lang.StringBuilder(),ch;while ((ch = fos.read()) != -1) {str.append(java.lang.Character(ch));}content = String(str.toString());fos.close();}return content;};
/* inform by @sliceofcode, licensed under MIT license, (c) 2016 sliceofcode | http://github.com/sliceofcode/inform */
var InformScriptWrapper,inform;inform={VERSION:"0.0.1"},InformScriptWrapper=function(){function InformScriptWrapper(script1){this.script=script1}return InformScriptWrapper.prototype.defined=function(name){var scope;return scope=this.script.scope,org.mozilla.javascript.ScriptableObject.hasProperty(scope,name)},InformScriptWrapper.prototype.real=function(){return this.script},InformScriptWrapper.prototype.set=function(name,value){return org.mozilla.javascript.ScriptableObject.putProperty(this.scope(),name,value)},InformScriptWrapper.prototype.name=function(){return this.script.name},InformScriptWrapper.prototype.call=function(name,args){return null==args&&(args=[]),org.mozilla.javascript.ScriptableObject.callMethod(this.scope(),name,args)},InformScriptWrapper.prototype.get=function(name){return org.mozilla.javascript.ScriptableObject.getProperty(this.scope(),name)},InformScriptWrapper.prototype.scope=function(){return this.script.scope},InformScriptWrapper}(),inform.cooperateAll=function(){var i,scriptsArray,scriptsArrayList;for(scriptsArrayList=net.zhuoweizhang.mcpelauncher.ScriptManager.scripts,scriptsArray=[],i=0;i<scriptsArrayList.size();)scriptsArray.push(new InformScriptWrapper(scriptsArrayList.get(i))),i++;return scriptsArray},inform.cooperateFind=function(name){var j,len,ref,script;for(ref=inform.scripts().all(),j=0,len=ref.length;len>j;j++)if(script=ref[j],clientMessage("FIND: Looking @ "+script+" ("+script.name()+")"),clientMessage("FIND: "+script.name()+" is "+name+"???"),script.name()==name)return script;return null},inform.cooperateExpect=function(name){var j,len,ref,script;for(ref=inform.scripts().all(),j=0,len=ref.length;len>j;j++)if(script=ref[j],script.name()===name)return!0;throw new Error("Expected to find "+name+", but it wasn't present")},inform.cooperateHas=function(name){var j,len,ref,script;for(ref=inform.scripts().all(),j=0,len=ref.length;len>j;j++)if(script=ref[j],script.name()===name)return!0;return!1},inform._scripts={all:inform.cooperateAll,find:inform.cooperateFind,expect:inform.cooperateExpect,has:inform.cooperateHas},inform.scripts=function(){return inform._scripts},module.exports=inform._scripts;

//Let's begin
var metals = inform.scripts().find("Metallurgy").get("metals");
var Metallurgy = {};
var data = JSON.parse(ModPE.readFile(ModPE.openTexturePackInputStream("assets/data/ModInfo.JSON")));





function newLevel(){BetterStorage.newLevel();metallurgyCM.newLevel();}function modTick(){BetterStorage.modTick();metallurgyCM.modTick();}function useItem(x,y,z,itemID,blockID,side,itemDat,blockDat){BetterStorage.useItem(x,y,z,itemID,blockID,side,itemDat,blockDat);metallurgyCM.useItem(x, y, z, itemID, blockID, side, itemDat, blockDat);}function destroyBlock(x,y,z,side){BetterStorage.destroyBlock(x,y,z,side);metallurgyCM.destroyBlock(x, y, z, side);}function leaveGame(){metallurgyCM.leaveGame();}
